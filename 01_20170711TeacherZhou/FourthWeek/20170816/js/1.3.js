...
//========================================定时器的时间: 0s(0)
// //->定时器时间是0s(0),不是立马执行;
// //->定时器的时间因子设置为零,也不是立马执行,每一个浏览器都有一个最小的反应时间(预估测试值: google最小的反应时间是5-6ms IE最小反应时间是10-13ms 我们一般设置的时候,最小值设置为17ms"性能会更好一些";有很多类库的定时器时间都写的17,也有部分用的13;),当我们设置的时间小于这个值,浏览器也是按照最小的时间处理的 =>也就是说,设置一个定时器,怎么着也要等一会执行,此时不等,继续执行下面的任务...
// var n = 12;
// setTimeout(function () {
//     n++;
//     //console.log(n);//->2.再打印13;
