...
//-------------------------------jQuery 原理
// jQuery 是基于构造函数模式创建的;jQuery就是一个类;在jQuery的原型上,提供了很多的方法;这些方法可以供JQ的实例使用;
// jQuery([selector],[context]) <=> $([selector],[context])//->其实就是获得一个类的实例
// selector: jQuery的选择器
// context: 当前获取元素的上下文(限定了获取的范围),如果不传递的话,默认是document;
// {jQuery.init}
...
//----------------------------
//=>JQ的方法一共分为两大部分
//1、把JQ作为类,在它原型上有很多的属性和方法,这些是供实例使用的(这里的方法一般都和DOM有些关系) $('#box').addClass('select');//->增加样式类名;  获取的是一个集合,为啥可以用addClass方法呢?一会儿讲;
//$()中第一个传一个字符串,是选择器;
